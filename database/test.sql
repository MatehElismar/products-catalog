...
-- Delete records from MetadataValues where FileLogId is in the temporary table
DELETE FROM MetadataValues
WHERE FileLogId IN (SELECT Id FROM #FileLogsToDelete)

-- Drop the temporary table
DROP TABLE #FileLogsToDelete
END
ELSE
BEGIN
DELETE FROM Files WHERE Id IN (SELECT FileId FROM #FilesToDelete)
END;
...
END;
GO
/****** Object:  StoredProcedure [dbo].[uspUpdateFile]    Script Date: 10/7/2024 7:23:39 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[uspUpdateFile]
    @Id uniqueidentifier,
    @DirectoryId uniqueidentifier,
    @IsParent bit,
    @ShouldBeClassified bit,
    @ClassificationStatus int,
	@PageNumber int,
	@PageCount int,
	@UpdateDate DATETIME
...
SET
    ...
    IsParent = @IsParent,
    ShouldBeClassified = @ShouldBeClassified,
